...
import { Asset, CheckRecord } from '../types';
import { DataService } from '../services/dataService';
import { ClipboardCheck, Check, X, AlertTriangle, Calendar, Activity, Zap, Monitor, Sparkles } from 'lucide-react';
...
const DailyCheck: React.FC = () => {
  ...
  useEffect(() => {
    ...
    DataService.getChecks().then(setRecentChecks);
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validation: If abnormal, note is required (optional but good for UX)
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center space-x-3 mb-6 no-print">
        ...
        <div>
          <h2 className="text-2xl font-bold text-slate-800">การตรวจเช็คประจำวัน (Daily Readiness)</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 no-print">
             <h3 className="font-bold text-xl text-slate-800">ประวัติการตรวจสอบ (History)</h3>
